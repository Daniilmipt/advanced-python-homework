...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser_structure = subparsers.add_parser("structure", help="Print workspace structure")
...
def draw_tree(tree: TaskTree):
    ...
    for item in dc:
        ...
        labelled[item[0].task.name] = "W: {0} \n T: {1}".format(item[0].workspace.name, item[0].task.name)
        labelled[item[1].task.name] = "W: {0} \n T: {1}".format(item[1].workspace.name, item[1].task.name)
