...
class Distributor:
    ...
    async def __call__(self, reader: StreamReader, writer: StreamWriter):
        '''
            Использовал writer.drain() на всякий случай, т.к. неизвестно сколько данных могут записываться.
            Кажется в нашей задаче он вообще не нужен, ибо по одной записи все записывается
        '''
        try:
            envelop_data = await Envelope.async_read(reader)
        except:
            writer.write(Envelope({'status': 'failed', 'error': 'Not Envelop format'}).to_bytes())
            await writer.drain()
        ...
        if 'command' in meta:
            match meta['command']:
                case 'run':
                    if 'task_path' in meta:
                        ...
                        envelope = await send_message(
                            Envelope(dict(command="run", task_path=meta['task_path']), data),
                            Distributor.server
                        )

                        task_result = envelope.meta['task_result']
                        writer.write(Envelope({"status": "success", 'task_result': task_result}).to_bytes())
                case 'structure':
                    Distributor.server = await get_server(self.servers)
                ...
                case 'powerfullity':
                    ...
                    writer.write(Envelope({"status": "success", 'powerfullity': sm}).to_bytes())
        else:
            writer.write(Envelope({'status': 'failed', 'error': 'KeyError: command'}).to_bytes())
...
def start_distributor_in_subprocess(host: str, port: int, servers: list[tuple[str, int]]) -> Process:
    my_thread = Process(target=start_distributor_general, args=(host, port, servers), daemon=True)
    my_thread.start()
    return my_thread
