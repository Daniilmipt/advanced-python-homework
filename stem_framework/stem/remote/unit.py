            ...
            match meta['command']:
                case 'run':
                    if 'task_path' in meta:
                        task = UnitHandler.workspace.find_task(meta['task_path'])
                        ...
                        Envelope({"status": "success", "task_result": task_result}).write_to(self.wfile)
                case 'structure':
                    dc = UnitHandler.workspace.structure()
                    dc["status"] = "success"
                    Envelope(dc).write_to(self.wfile)
                case 'powerfullity':
                    Envelope({"status": "success", 'powerfullity': UnitHandler.powerfullity}).write_to(self.wfile)
                case 'stop':
                    self.server.shutdown()
                    self.server.server_close()
        else:
            Envelope({'status': 'failed', 'error': 'KeyError: command'}).write_to(self.wfile)
...
def start_unit_in_subprocess(workspace: IWorkspace, host: str, port: int,
    ...
    my_thread = Process(target=start_unit, args=(workspace, host, port, powerfullity), daemon=True)
    my_thread.start()
    return my_thread
